...
(defun context-navigator-render--format-line (item icon-fn)
...
'context-navigator-item for later retrieval at point."
  (let* ((key (context-navigator-model-item-key item))
         ...
         (col-left (string-trim (mapconcat #'identity
                                           (delq nil (list (format "[%s%s]" type-ch state)
         ...
         (line (if col-right
                   (format "%-48s  %s" col-left col-right)
                 col-left)))
...
(defun context-navigator-render-build-lines (items header &optional icon-fn)
...
ICON-FN is a function (item -> string|nil) to decorate items."
  (let* ((title (or header "Context"))
    ...
    (append (list hl sep)
            (mapcar (lambda (it) (context-navigator-render--format-line it icon-fn))
                    items))))
...
(defun context-navigator-render-apply-to-buffer (buffer lines)
  ...
  (with-current-buffer buffer
    (let* ((inhibit-read-only t)
           ...
           (changed (not (and (= (buffer-size) (length new-text))
