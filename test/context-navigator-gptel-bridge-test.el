...
;; apply should ignore disabled items on add (green after fix)
(ert-deftest context-navigator-gptel/apply-ignores-disabled-on-add ()OK  ~/Code/context-navigator/test/context-navigator-gptel-bridge-test.el
              ...
              (let* ((enabled (context-navigator-item-create :type 'file :path "/tmp/en.txt" :name "en" :enabled t))
                ...
                (cl-letf (((symbol-function 'context-navigator-gptel--add-item)
                  ...
                  (let ((res (context-navigator-gptel-apply (list enabled disabled))))
                    (ignore res)
                    ;; EXPECTED after fix: only enabled item should be added
                    ;; Current behavior: both will be attempted (test should FAIL).
                    (should (equal (sort added #'string<) (list (context-navigator-model-item-key enabled)))))))))
